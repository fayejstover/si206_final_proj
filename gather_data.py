...
def insert_NBA_data():
    ...
    for row in data:
        ...
        c.execute(
            "UPDATE NBAplayers SET RANK = ?, PLAYER = ?, TEAM_ID = ?, TEAM = ? WHERE PLAYER_ID = ?",
            (rank, player, team_id, team, player_id))

        if c.rowcount == 0:
            c.execute(
                "INSERT INTO NBAplayers (PLAYER_ID, RANK, PLAYER, TEAM_ID, TEAM) VALUES (?, ?, ?, ?, ?)",
                (player_id, rank, player, team_id, team))

        c.execute(
            "INSERT OR REPLACE INTO NBAstats (PLAYER_ID, PTS, REB, AST, STL, BLK, FGM, FGA, FTM, FTA, TOV, GP) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (player_id, pts, reb, ast, stl, blk, fgm, fga, ftm, fta, tov, gp))
...
'''
    c.execute("INSERT INTO NBAplayers (PLAYER_ID, RANK, PLAYER, TEAM_ID, TEAM) VALUES (?, ?, ?, ?, ?)",
                  (player_id, rank, player, team_id, team))
    c.execute("INSERT INTO NBAstats (PLAYER_ID, PTS, REB, AST, STL, BLK, FGM, FGA, FTM, FTA, TOV, GP) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                  (player_id, pts, reb, ast, stl, blk, fgm, fga, ftm, fta, tov, gp))
    conn.commit()
    conn.close()
'''
